 /* 
  * Student name and id: Lingfei Li 9074068637
 */
...
{
    //Tested item:
    //allocPage, insertRecord - getRecord

    std::cout<<"Test 1:\n";
	//Allocating pages in a file...
    std::cout<<"Allocating pages in a file...\n";
	for (i = 0; i < num; i++)
	{
...
void test7() {
	...
	{
		bufMgr->readPage(file1ptr, pid[i], page);
		printf("test.1 Page %d %7.1f", pid[i], (float)pid[i]);
		printf("\n");
		sprintf((char*)&tmpbuf, "test.1 Page %d %7.1f", pid[i], (float)pid[i]);
		if(strncmp(page->getRecord(rid[i]).c_str(), tmpbuf, strlen(tmpbuf)) != 0)
		{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        {
            bufMgr->readPage(file1ptr, i, page);
            PRINT_ERROR("ERROR :: The page should be disposed");
        }
    ...
    //Manuallly check if all frames are free
    bufMgr->printSelf();
