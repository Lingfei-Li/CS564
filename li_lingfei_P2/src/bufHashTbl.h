...
class BufHashTbl
{
 ...
 public:
  ...
  void insert(const File* file, const PageId pageNo, const FrameId frameNo);
     ...
     * the hash table). If so, assign the value to frameNo and return true. 
	 *
	 * @param file  	File object
	 * @param pageNo	Page number in the file
	 * @param frameNo   Frame number reference that is assigned if (file, pageNo) is present
	 * @return  	    true if (file, pageNo) is present. Else false. 
	 */
